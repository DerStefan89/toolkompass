/**
 * Datei: app/admin/tools/screenshot-action.ts
 *
 * Zweck: Server Action für Screenshot-Upload via Supabase Storage.
...
export async function uploadToolScreenshot(
  ...
  if (file.size > MAX_BYTES) return { error: 'Die Datei ist größer als 3 MB.' }

  const ext  = file.name.split('.').pop()?.toLowerCase() ?? 'png'
  const path = `${toolId}/screenshot.${ext}`
